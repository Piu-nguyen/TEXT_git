//Chuong trinh C/C++ cho danh sach lien ket vong quan li sanh sach so nguyen
...
void List::sapXep(){
	...
	for(SoNguyen *i=head;i->next!=head;i=i->next){
		for(SoNguyen *j=i->next;j!=head;j=j->next){
			if(i->data>j->data){
				int temp=i->data;
				i->data=j->data;
				j->data=temp;
